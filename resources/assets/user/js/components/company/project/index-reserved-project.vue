...
<script>
    export default {
        ...
        methods: {
            getProjectList() {
                axios.get(`/companies/reserved-project/${this.$route.params.id}`).then(response => this.projects = response.data).catch(error => this.errorNotify(error.response.data.message));
            },
            jdate(date) {
