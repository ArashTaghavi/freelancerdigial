    ...
    <div class="card">
        ...
        <div class="stepwizard">
            <div class="stepwizard-row setup-panel">
                ...
                <div class="stepwizard-step col-xs-3">
                    <router-link :to="`/companies/${$route.params.id}/attachment`" type="button" class="btn btn-default btn-circle" disabled="disabled">2</router-link>
                    <p><small>مدارک</small></p>
                </div>
                <div class="stepwizard-step col-xs-3">
                    <router-link :to="`/companies/${$route.params.id}/work-sample`" type="button" class="btn btn-default btn-circle" disabled="disabled">3</router-link>
                    <p><small>نمونه کار</small></p>
                </div>
                <div class="stepwizard-step col-xs-3">
                    <router-link :to="`/companies/${$route.params.id}/member`" type="button" class="btn btn-circle btn-default btn-success">4</router-link>
...
<script>
    export default {
        ...
        methods: {
            handleSubmit() {
                if (this.member_id === 0) {
                    axios.post(`/companies/${this.$route.params.id}/member`, this.form)
                        .then(response => {
                    ...
                    axios.put(`/companies/member/${this.member_id}`, this.form)
                        .then(response => {
                            this.successNotify(response.data.message);
                            this.getMemberList();
                            this.form = {
            ...
            afterFetchItem(response) {
                ...
                this.breadcrumb = [
                    ...
                    {title: this.company.title, to: `/companies/${this.company.id}/member`},
                ];
            },
            getMemberList() {
                axios.get(`/companies/member-list/${this.$route.params.id}`)
                    .then(response => {
            ...
            handleUpdate(id) {
                ...
                axios.get(`/companies/member/${id}`).then(response => {
            ...
            handleDelete(id) {
                this.deleteConfirm()
                    .then(confirm => {
                        if (confirm) {
                            axios.delete(`/companies/member/${id}`).then(response => {
                                this.successNotify(response.data.message);
                                this.getMemberList();
                            }).catch(error => this.errorNotify(error.response.data.message));
                        }
            ...
            handleDeleteFile(id, file) {
                this.deleteConfirm()
                    .then(confirm => {
                        if (confirm) {
                            axios.delete(`/companies/member/single/${id}?file=${file}`).then(response => {
                                this.successNotify(response.data.message);
                                this.getMemberList();
                            }).catch(error => this.errorNotify(error.response.data.message));
                        }
